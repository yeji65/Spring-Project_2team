								...
								<tbody>
									<c:forEach items="${adminOrderDetail}" var="order">
										<tr>
											...
											<c:choose>
												<c:when test="${order.order_status eq 'member_waiting'}">
													<td>
														<a href="adminConfirm.do?order_status=admin_approve&order_no=${order.order_no}">승인 /</a>
														<a href="adminConfirm.do?order_status=admin_cancel&order_no=${order.order_no}"> 반려</a>
													</td>
												</c:when>
												<c:otherwise>
													<td>완료</td>
