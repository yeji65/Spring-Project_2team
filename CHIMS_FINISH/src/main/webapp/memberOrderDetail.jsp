								...
								<tbody>
									<c:forEach items="${memberOrderDetail}" var="order">
										<tr>
											...
											<c:choose>
												<c:when test="${order.order_status eq 'admin_approve'}">
													<td>
														<a href="memberConfirm.do?order_status=member_ok&order_no=${order.order_no}&product_code=${order.product_code}&dept_no=${order.dept_no}&order_product_cnt=${order.order_product_cnt}">확인</a>
													</td>
												</c:when>
												<c:when test="${order.order_status eq 'admin_cancel'}">
													<td>반려</td>
												</c:when>
											</c:choose>
										</tr>
